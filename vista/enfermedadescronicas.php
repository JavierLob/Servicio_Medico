<script>
    function buscar(id)
    {
        window.location.href="?vista=consultar_enfermedadescronicas&id="+id;
    }
    function desactivar(id)
    {
        if(confirm("¿Está seguro que desea desactivar las enfermedades cronicas seleccionadas?"))
        {
          document.getElementById("cam_idtenfermedadescronicas").value=id;
            document.getElementById("cam_estatusenfermedadescronicas").value='0';
    ...
    function activar(id)
    {
        ...
        {
            document.getElementById("cam_idtenfermedadescronicas").value=id;
            document.getElementById("cam_estatusenfermedadescronicas").value='1';
            document.getElementById("cam_operacion").value='activar_enfermedadescronicas';
            document.form_enfermedadescronicas.submit();
        }
    }
</script>    
<h1 class="page-header">Enfermedades Cronicas</h1>
...
<!-- FIN: RECOMENDACION -->
<a class="btn btn-success" id="btn_registrar" href="?vista=registrar_enfermedadescronicas"><i class="fa fa-plus"></i> Registrar enfermedad cronica</a>
<form role="form" class="form" action="../control/c_enfermedadescronicas.php" method="POST" name="form_enfermedadescronicas" role="form" class="form">
    ...
    <input type="hidden"  name="estatusenfermedad" id="cam_estatusenfermedadescronicas"/>
    <table class="cell-border compact hover stripe" id="filtro">
        ...
        <?php
                ...
                $lobjEnfermedadescronicas=new claseEnfermedadCronicas;
                $laEnfermedadesCronicas=$lobjEnfermedadescronicas->listar();
                for($i=0;$i<count($laEnfermedadesCronicas);$i++)
                {
                    $estatus=($laEnfermedadesCronicas[$i]['estatusenfermedad']==1)?'Activo':'Inactivo';
                    echo '<tr>';
                    echo '<td>'.$laEnfermedadesCronicas[$i]['idtenfermedadescronicas'].'</td>';
                    echo '<td>'.$laEnfermedadesCronicas[$i]['enfermedadcronica'].'</td>';
                    echo '<td>'.$estatus.'</td>';
                    echo '<td>';
